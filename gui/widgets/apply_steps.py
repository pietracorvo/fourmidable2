...
class ApplySteps(QWidget):
    ...
    def create_signal_from_parameters(self):
        if self.params.child("Signal", "Custom signal", "Use custom signal").value():
            ...
            nb_cycles = self.params.child("Signal", "Number of repetitions").value()
            nb_steps = self.params.child("Signal", "Number of points per repetition").value()
            signal_types = {'sinus': lambda x: np.sin(x),
                            'triangle': lambda x: 2 / np.pi * np.arcsin(np.sin(x))}
            signal_generator = signal_types[self.params.child("Signal", "Signal type").value()]
            amplitudes = [0., 0., 0.]
            amplitudes[0] = self.params.child("Signal", "Amplitudes", "X").value()
            amplitudes[1] = self.params.child("Signal", "Amplitudes", "Y").value()
            amplitudes[2] = self.params.child("Signal", "Amplitudes", "Z").value()
            phases = [0., 0., 0.]
            phases[0] = np.deg2rad(self.params.child("Signal", "Phases", "X").value())
            phases[1] = np.deg2rad(self.params.child("Signal", "Phases", "Y").value())
            phases[2] = np.deg2rad(self.params.child("Signal", "Phases", "Z").value())
            offsets = [0., 0., 0.]
            offsets[0] = self.params.child("Signal", "Offsets", "X").value()
            offsets[1] = self.params.child("Signal", "Offsets", "Y").value()
            offsets[2] = self.params.child("Signal", "Offsets", "Z").value()
            seq = []
