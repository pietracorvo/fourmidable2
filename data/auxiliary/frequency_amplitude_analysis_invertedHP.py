...
def plot_fft(data_file):
    ...
    phase_diff = data["hexapole_phase"] - data["hallprobe_phase"]
    ...
    cbar.set_label('Input amplitude [mT]')
    plt.xlabel('Input frequency [Hz]')
    plt.ylabel('Coil phase difference')
    plt.grid()
...
def collect_fft(folder_in, file_out, invert_hp=True, invert_fun=None):
            ...
            for loop in grp.keys():
                ...
                for device in device_grp.keys():
                    if device in {"hexapole", "hallprobe"}:
                        ...
                        if invert_hp and invert_fun is not None and device == 'hallprobe':
                            data[device][loop] = invert_fun(device_data[:, 1:])  # remove time
                        else:
...
if __name__ == "__main__":
    ...
    invert_fun = lambda x, A=9.6, b=1.01, c=3.49: A * np.arctanh(-b * x / 2) + c * x
    collect_fft(folder, file_fft, invert_fun=invert_fun, invert_hp=True)
