...
import time
import h5py
from data.signal_generation import get_random_signal


def loop_worker(mk, stop_event, cutoff_freq=100, n_loops=3, skip_loops=0, max_period=5, min_period=0.1, filename=None):
    try:
        ...
        signal = get_random_signal(cutoff_freq=cutoff_freq, min_amp=0)
        ...
        with h5py.File(filename, 'w') as f:
            ...
            # reset the outputs
            magnet.stage_data(
                signal_generation.zeros_signal(), 1, autostart=True)
...
if __name__ == "__main__":
    ...
    n_loops = 3
    skip_loops = 0
    filename = 'MagCalibRecurr_' + time.strftime("%Y%m%d-%H%M%S") + '.h5'
    ...
    with Moke() as mk:
        ...
        saving_thread = threading.Thread(
            target=loop_worker, args=(mk, stop_event), kwargs={"filename": filename})
